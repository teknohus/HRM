...
use App\Mail\BirthdayEmailSend;
...
class EmployeeController extends Controller
{
    ...
    //     return view('employee.probation');
    // }
    public function anniversary(){
        if (Auth::user()->type == 'company'){
            $subject = 'Happy Work Anniversary';
            $today = today();
            $anniver = DB::table('employees')
                        ->whereMonth('company_doj', '=', $today->format('m'))
                        ->whereDay('company_doj', '>=', $today->format('d'))
                        ->orderByRaw('MONTH(company_doj), DAYOFMONTH(company_doj)')->get();
            $remainingDays = [];
            $companyName = 'Teknohus';

            $workedYears = 0;
            foreach ($anniver as $employee){
                $doj = Carbon::parse($employee->company_doj);
                $dojmail = $employee->email;
                $employeeName = $employee->name;
                $workedYears = $doj->diffInYears($today);
                // dd($workedYears);
                // if ($doj->month < $today->month || ($doj->month == $today->month && $doj->day < $today->day)) {
                //     $Anniversary = $doj->copy()->addYear();
                // } else {
                //     $Anniversary = $doj->copy();
                // }
                $Anniversary = $doj->copy();
                if ($Anniversary->month == $today->month) {
                    $remainingDays[$employee->id] = $today->diffInDays($Anniversary);
                } else {
                    $remainingDays[$employee->id] = null;
                }
                if ($Anniversary->month == $today->month && $Anniversary->day == $today->day && ($workedYears != 0))
                {
                    $employeeId = $employee->id;
                    $checkemailInserted = DB::table('emailcheckannivers')->where('user_id', $employeeId)->first();
                    $HR = "hr@teknohus.com";
                    if (!$checkemailInserted || $checkemailInserted->created_at < now()->startOfDay()) {
                        try {
                            config([
                                    'mail.driver' => 'smtp',
                                    'mail.host' => 'mail.teknohus.com',
                                    'mail.port' => 465,
                                    'mail.encryption' => 'tls',
                                    'mail.username' => $HR,
                                    'mail.password' => 'eshaesha1122',
                                    'mail.from.address' => $HR,
                                    'mail.from.name' => "HR Teknohus",
                                ]);
                        EmailJobs::dispatch($dojmail,$subject,$employeeName, $companyName,$workedYears);    
                        } catch (\Exception $e) {
                        ...
                        if (!$checkemailInserted) {
                            DB::table('emailcheckannivers')->insert(['user_id' => $employeeId, 'status' => true, 'created_at' => now()]);
                        } else {
                            DB::table('emailcheckannivers')->where('user_id', $employeeId)->update(['created_at' => now()]);
                        }
                    }
                }
            }
            return view('employee.anniversary' , compact('anniver','remainingDays','workedYears'));
        }else{
    ...
    public function birthday()
    {
        if (Auth::user()->type == 'company') {
            ...
            $upcomingBirthdays = DB::table('employees')
                        ...
                        ->orderByRaw('MONTH(dob), DAYOFMONTH(dob)')->get();
            $nextMonth = DB::table('employees')
                            ->whereMonth('dob', '=', $nextMonth->format('m'))
                            ->orderByRaw('DAYOFMONTH(dob)')->get();
            ...
            foreach ($upcomingBirthdays as $employee) {
                ...
                $dobemail = $employee->email;
                $employeeName = $employee->name;
                $nextBirthday = $dob->copy()->setYear($today->year);
                if ($nextBirthday->lt($today)) {
                    $nextBirthday->addYear();
                }
                $remainingDays[$employee->id] = $today->diffInDays($nextBirthday);
                if ($dob->month == $today->month && $dob->day == $today->day ){

                    $employeeId = $employee->id;
                    $checkemailInserted = DB::table('emailchecks')->where('user_id', $employeeId)->first();
                    $HR = "hr@teknohus.com";
                    if (!$checkemailInserted || $checkemailInserted->created_at < now()->startOfDay()) {
                          try {
                            config([
                                ...
                                // Mail::to($HRmail)->send(new HRmail($popup , $popupremain , $anniversarypopup , $popupanniversarypopupremain));
                                Mail::to($dobemail)->send(new BirthdayEmailSend($subject, $employeeName, $companyName));
                        ...
                        if (!$checkemailInserted) {
                            DB::table('emailchecks')->insert(['user_id' => $employeeId, 'status' => true, 'created_at' => now()]);
                        } else {
                            DB::table('emailchecks')->where('user_id', $employeeId)->update(['created_at' => now()]);
                        }
                    }
                }
            }
            return view('employee.birthday',  compact('upcomingBirthdays', 'nextMonth' , 'remainingDays'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function create()
    {
        if (\Auth::user()->can('Create Employee')) {
            ...
            $probation        = DB::table('probations')->where('user_id','probation')->get();

            return view('employee.create', compact('probation','employees', 'employeesId', 'departments', 'designations', 'documents', 'branches', 'company_settings'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    public function store(Request $request)
    {
        if (\Auth::user()->can('Create Employee')) {
            ...
            // dd($user_id);
            $probation =  DB::table('probations')->insert(
                [
                    'user_id'=>$employee->id,
                    'probation'=>$request['probation_id'],
                ]);
            if ($request->hasFile('document')) {
                foreach ($request->document as $key => $document) {
                    $image_size = $request->file('document')[$key]->getSize();
    ...
    public function edit($id)
    {
        ...
        if (\Auth::user()->can('Edit Employee')) {
            ...
            $user         = Employee::find($employee->user_id);
            $employeesId  = \Auth::user()->employeeIdFormat($employee->employee_id);
            // dd($id);
            $probation    = DB::table('probations')->where('user_id',$id)->get('probation');
            

            return view('employee.edit', compact('user','probation','employee', 'employeesId', 'branches', 'departments', 'designations', 'documents'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (\Auth::user()->can('Edit Employee')) {
            ...
            // $employeeidd = Employee::findOrFail($employee->user_id);
            // dd($id);
            $probationExists = DB::table('probations')->where('user_id', $id)->exists();
            // dd($employeeidd);
            if ($probationExists) {
                $probation = DB::table('probations')
                    ->where('user_id', $employee->id)
                    ->update(['user_id' => $employee->id, 'probation' => $request['probation_id']]);
            } else {
                // dd($employee->id);
                $probation = DB::table('probations')
                    ->insert(['user_id' => $employee->id, 'probation' => $request['probation_id']]);
            }
            $employee = Employee::findOrFail($id);
            $input    = $request->all();
    ...
    public function show($id)
    {
        ...
        if (\Auth::user()->can('Show Employee')) {
            ...
            $probation =  DB::table('probations')->where('user_id',$eId  )->get('probation');

            //     $employee     = Employee::find($empId);
            // $branch= Branch::where('created_by', \Auth::user()->creatorId())->get()->pluck('name', 'id');

            return view('employee.show', compact('eId','probation','employee', 'employeesId', 'branches', 'departments', 'designations', 'documents'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
