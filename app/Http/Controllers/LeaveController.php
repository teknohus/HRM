...
class LeaveController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('Create Leave')) {
            ...
            if($request->leave_full_half =='half'){
                ...
                if ($leave_type->days >= $total_leave_days) {
                    ...
                    $leave->leave_full_half   = $request->leave_full_half;
                ...
                if ($leave_type->days >= $total_leave_days) {
                    ...
                    if($request->leave_full_half == 'half'){
                        ...
                        if ($leave_type->days >= $total_leave_days) {
                            ...
                            DB::table('leaves')->where('employee_id', $request->leave_type_id )->update([
                                'leave_type_id'  =>   $request->leave_type_id,
                                'leave_full_half' => $request->leave_full_half,
    ...
    public function changeaction(Request $request)
    {
        ...
        if ($leave->status == 'Approved' && $leave->leave_full_half == 'half') {
            $startDate               = new \DateTime($leave->start_date);
            $endDate                 = new \DateTime($leave->end_date);
            $endDate->add(new \DateInterval('P1D'));
            $total_leave_days        = 0.5;
