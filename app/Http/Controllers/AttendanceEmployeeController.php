...
class AttendanceEmployeeController extends Controller
{
    public function index(Request $request)
    {
        if (\Auth::user()->can('Manage Attendance')) {
            ...
            if (\Auth::user()->type == 'employee') {
             ...
             $myattendanceEmployee  = AttendanceEmployee::get();
             //total worked hours
             $tableinfo = DB::table('employee_worked_hours')->get();
             $employeeIds = $tableinfo->pluck('employee_id')->toArray();
            // //  dd( $tableinfo->pluck('employee_id'),$employeeIds );
            //  $names = DB::table('employees')->whereIn('id', $employeeIds)->pluck('name');

             $tableinfo = DB::table('employee_worked_hours')
             ->join('employees', 'employee_worked_hours.employee_id', '=', 'employees.id')
             ->select('employee_worked_hours.date', 'employee_worked_hours.total_working_hours', 'employees.id', 'employees.name')
             ->get();        

            return view('attendance.index', compact('tableinfo','attendanceEmployee', 'branch', 'department'));
        } else {
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!empty($todayAttendance) && $todayAttendance->clock_out == '00:00:00') {
            ...
            if (Auth::user()->type == 'employee') {
                ...
                // dd($attendanceEmployee->clock_out);
                $totalWorkingHours = '00:00:00'; 
                $clockIn = strtotime($attendanceEmployee->clock_in);
                $clockOut = strtotime(date("Y-m-d H:i:s")); // Current time
                $totalWorkingSeconds = $clockOut - $clockIn;
                // Convert seconds to hours, minutes, and seconds
                $hours = floor($totalWorkingSeconds / 3600);
                $mins = floor(($totalWorkingSeconds % 3600) / 60);
                $secs = $totalWorkingSeconds % 60;
    
                // Format the total working hours
                $currentWorkingHours  = sprintf('%02d:%02d:%02d', $hours, $mins, $secs);
                $attendances = AttendanceEmployee::where('employee_id', $employeeId)->where('date', $date)->whereNotNull('clock_out')->get();
                ...
                $totalWorkingHours = Carbon::parse("00:00:00")->addMinutes($totalWorkingHours)->format('H:i:s');
                // dd($totalWorkingHours);
                $attendanceEmployee->clock_out = date("H:i:s");
                $attendanceEmployee->working_hours = $currentWorkingHours ;
                $attendanceEmployee->save();
                //total working hours of a day
                $employeeId = $attendanceEmployee->employee_id;
                $date = $attendanceEmployee->date;
                $check = DB::table('employee_worked_hours')->where('employee_id', $employeeId)->where('date', $date)->first();
            
            if ($check) {
                DB::table('employee_worked_hours')->where('employee_id', $employeeId)->where('date', $date)->update(['total_working_hours' => $totalWorkingHours]);
            } else {
                DB::table('employee_worked_hours')->insert(['employee_id' => $employeeId, 'total_working_hours' => $totalWorkingHours, 'date' => $date]);
            }
                ...
                $attendanceEmployee->save();
                $clockInToday = AttendanceEmployee::where('date', $request->date)->get(['employee_id','working_hours']);
        ...
        if ($check) {
            ...
            return redirect()->back()->with('success', __('Clock Out'));
        }
    }
    ...
    public function attendance(Request $request)
    {
        ...
        $startTime = Utility::getValByName('company_start_time');
        $endTime = Utility::getValByName('company_end_time');
        // Clock out previous attendance record if exists
        $previousAttendance = AttendanceEmployee::orderBy('id', 'desc')->where('employee_id', '=', $employeeId)->where('clock_out', '=', '00:00:00')->first();

        if ($previousAttendance != null) {
            $previousAttendance->clock_out = $endTime;
