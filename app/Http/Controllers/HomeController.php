...
class HomeController extends Controller
{
    ...
    public function popup(){
        ...
        // $result = $empolyeeBirthday->birthday();
        // dd($today);
    }
    public function index()
    {
        if (Auth::check()) {
            ...
            if ($user->type == 'employee') {
                ...
                $employee = Employee::where('user_id', $user->id)->get('user_id')->first(); 
                $employeeattId = AttendanceEmployee::where('created_by', $employee->user_id )->get('employee_id')->first(); 
                if ($employeeattId) {
                    if ($employee) {
                        $officeWorkedHours = DB::table('employee_worked_hours')
                                               ->where('employee_id', $employeeattId->employee_id)
                                               ->where('date', $date)
                                               ->first(['total_working_hours']);
                                               
                        $totalWorkingHours = $officeWorkedHours ? $officeWorkedHours->total_working_hours : '00:00:00';
                    } else {
                ...
                $emID = DB::table('employees')->where('user_id', $employee->user_id )->select('id','name')->first();
                // dd($emID);
                $birthday = DB::table('employees')->where('id', $emID->id )->whereMonth('dob', '=', $today->month)
                                 ->whereDay('dob', '=', $today->day)->first();
                $anniversary = DB::table('employees')->where('id',$emID->id)
                                ->whereYear('company_doj', '<>', $today->year)
                                ->whereMonth('company_doj', '=',$today->month)
                                ->whereDay('company_doj', '=', $today->day)->get(['name','company_doj']);
                $currentDay = Carbon::now();
                $probationDay = DB::table('probations')->where('user_id', $emID->id)->first('probation');

                $probation = 'false';
                
                if ($probationDay !== null && $probationDay->probation !== null) {
                    $probationDate = Carbon::parse($probationDay->probation);
                    $probationToday = Carbon::today();
                    if ($probationDate->isSameDay($probationToday)) {
                        $probation = 'true';
                    }
                }
                if ($anniversary->isNotEmpty()) {
                    $firstEmployee = $anniversary->first();
                    $doj = Carbon::parse($firstEmployee->company_doj);
                    $workedYears = $doj->diffInYears($today);
                }else{
                    $workedYears = 0;
                }
                return view('dashboard.dashboard', compact('probation','workedYears','emID','anniversary','birthday','totalWorkingHours','arrEvents', 'announcements', 'employees', 'meetings', 'employeeAttendance', 'officeTime'));
                ...
                $notClockIns = Employee::where('created_by', '=', \Auth::user()->creatorId())->whereIn('id', $notClockIn)->get();
                ...
                $probation = DB::table('probations')->whereMonth('probation', $currentMonth) ->get(['probation', 'user_id']);
                $probationIds = $probation->pluck('user_id');
                $pro_names = DB::table('employees')->whereIn('id', $probationIds)->pluck('name', 'id');
              
                $employeeController = new EmployeeController();
                $result = $employeeController->birthday();
                $resultt = $employeeController->anniversary();
                // dd($result['upcomingBirthdays']);
                $popup = $result['upcomingBirthdays'] ;
                $popupremain = $result['remainingDays'] ;
                $anniversarypopup = $resultt['anniver'] ;
                $anniversaryworked = $resultt['workedYears'] ;
                $popupanniversarypopupremain = $resultt['remainingDays'] ;
                $HR = "hr@teknohus.com";
                $company = User::where('type','company')->get();
                $HRmail = User::where('type','hr')->get();
                if (!Cache::has('birthday_email_sent')) {
                    if (\Auth::user()->type == 'company' || \Auth::user()->type == 'hr'){
                        try {
                            config([
                                    'mail.driver' => 'smtp',
                            ...
                            if($company !== null && filter_var($company, FILTER_VALIDATE_EMAIL)){
                                Mail::to($company)->send(new HRmail($popup , $popupremain , $anniversarypopup , $popupanniversarypopupremain));
                            }
                        } catch (\Exception $e) {
                        ...
                        try {
                            ...
                            if($HRmail !== null && filter_var($HRmail, FILTER_VALIDATE_EMAIL)){
                        ...
                        Cache::put('birthday_email_sent', true, now()->addDay());
                    }
                }
                return view('dashboard.dashboard', compact('anniversaryworked','pro_names','probation','popupremain','popupanniversarypopupremain','anniversarypopup','popup','arrEvents', 'announcements', 'employees', 'activeJob', 'inActiveJOb', 'meetings', 'countEmployee', 'countUser', 'countTicket', 'countOpenTicket', 'countCloseTicket', 'notClockIns', 'accountBalance', 'totalPayee', 'totalPayer', 'users', 'plan', 'storage_limit'));
            }
